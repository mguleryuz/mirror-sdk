...
export function mediaSetup(ogObject: OgObjectInteral) {
  ...
  if (
    ...
    ogObject.ogImageProperty =
      ogObject.ogImageSecureURL.length !== 0
        ? ogObject.ogImageSecureURL
        : ogObject.ogImageProperty
    // fall back to ogImageURL if ogImageProperty isn't set
    ogObject.ogImageProperty =
      ogObject.ogImageProperty.length !== 0
        ? ogObject.ogImageProperty
        : ogObject.ogImageURL
  ...
  if (
    ...
    ogObject.twitterImageSrc = ogObject.twitterImageSrc
      ? ogObject.twitterImageSrc
      : []
    // eslint-disable-next-line max-len
  ...
  const twitterImages: TwitterImageObject[] = zip(
    ...
    .filter(
      (value: TwitterImageObject) => value.url !== undefined && value.url !== ''
    )
  ...
  if (
    ogObject.musicSongProperty ||
    ogObject.musicSongTrack ||
    ogObject.musicSongDisc ||
    ogObject.musicSongUrl
  ) {
    ogObject.musicSongUrl = ogObject.musicSongUrl ? ogObject.musicSongUrl : []
    ogObject.musicSongProperty = ogObject.musicSongProperty
      ? ogObject.musicSongProperty
      : ogObject.musicSongUrl // deafult to musicSongUrl
    ogObject.musicSongTrack = ogObject.musicSongTrack
      ? ogObject.musicSongTrack
      : []
    ogObject.musicSongDisc = ogObject.musicSongDisc
      ? ogObject.musicSongDisc
      : []
  }

  // format music songs and limit to 10
  const musicSongs: MusicSongObject[] = zip(
    ogObject.musicSongProperty,
    ogObject.musicSongTrack,
    ogObject.musicSongDisc
  )
    ...
    .filter(
      (value: MusicSongObject) => value.url !== undefined && value.url !== ''
    )
