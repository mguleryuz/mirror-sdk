...
export function fallback(
  ...
  if (doesElementExist('meta', 'charset', $)) {
    ...
    doesElementExist('head > meta[http-equiv="content-type"]', 'content', $)
  ) {
    const content = $('head > meta[http-equiv="content-type"]').attr('content')
    const charsetRegEx = /charset=([^()<>@,;:"/[\]?.=\s]*)/i
    // @ts-ignore
    ogObject.charset = charsetRegEx.test(content)
      ? // @ts-ignore
        charsetRegEx.exec(content)[1]
      : 'UTF-8'
  } else if (body) {
