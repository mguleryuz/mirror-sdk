// @ts-expect-error types pkg is not installed
 ...
 *
 */
const coerceUrl = (url: string): string =>
  /^(f|ht)tps?:\/\//i.test(url) ? url : `http://${url}`
...
export function isThisANonHTMLUrl(url: string): boolean {
  ...
  return invalidImageTypes.some((type: string): boolean =>
 ...
 *
 */
export function removeNestedUndefinedValues(object: { [key: string]: any }): {
  [key: string]: any
} {
 ...
 *
 */
export function optionSetup(ogsOptions: OpenGraphScraperOptions): {
 ...
 *
 */
export function isCustomMetaTagsValid(
  ...
  customMetaTags.forEach((customMetaTag) => {
    if (typeof customMetaTag === 'object') {
      if (
        !('fieldName' in customMetaTag) ||
        typeof customMetaTag.fieldName !== 'string'
      )
        result = false
      if (
        !('multiple' in customMetaTag) ||
        typeof customMetaTag.multiple !== 'boolean'
      )
        result = false
      if (
        !('property' in customMetaTag) ||
        typeof customMetaTag.property !== 'string'
      )
        result = false
    } else {
      result = false
